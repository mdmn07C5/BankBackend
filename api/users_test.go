...
func TestCreateUserAPI(t *testing.T) {
		...
		},
		{
			...
			buildStubs: func(store *mockdb.MockStore) {
				store.EXPECT().
					CreateUser(gomock.Any(), gomock.Any()).
					Times(1).
					Return(db.User{}, &pq.Error{Code: util.UniqueViolation}) //unique_violation
			},
			checkResponse: func(t *testing.T, recorder *httptest.ResponseRecorder) {
	...
	for i := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			request, err := http.NewRequest(http.MethodPost, "/users", bytes.NewReader(jsonBytes))
			require.NoError(t, err)
	...
	}{
		{
			...
			buildStubs: func(store *mockdb.MockStore) {
				store.EXPECT().
					...
					Times(1).
					Return(user, nil)
				// TODO: make test stronger, replace Any with more specific object, maybe write custom matcher.
				store.EXPECT().
